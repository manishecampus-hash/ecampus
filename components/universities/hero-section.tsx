...
import { Award, ChevronLeft, CheckCircle2, Sparkles } from "lucide-react";
...
export default function HeroSection({ university }: HeroSectionProps) {
  // Clean dynamic text parsing avoiding trailing multi-bar repetition bugs
  const uniName = university?.name || "Amity";
  const uniFullName = university?.fullName || "Amity University Online";
  const defaultHeroImage =
    "https://images.unsplash.com/photo-1523240795612-9a054b0db644?w=800&auto=format&fit=crop&q=80";

  // If the dynamic university data name string already includes the word "Online",
  // prevent layout concatenation from doubling it up inside the DOM node.
  const hasOnlineWord = uniName.toLowerCase().includes("online");
  const displayHeading = hasOnlineWord ? uniName : `${uniName} Online`;

  return (
    /* ============================================================
        HERO SECTION — FIXED PATTERN ACCENT ARCHITECTURE
       ============================================================ */
    <section className="relative overflow-hidden border-b border-slate-100 bg-slate-50 pt-12 pb-14 lg:pt-14 lg:pb-16">
      {/* Background SVG grid — purely decorative background framework styling */}
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        <div className="grid items-center gap-12 md:grid-cols-2">
          {/* LEFT COLUMN — Text Content Matrix Block */}
          <div className="space-y-6">
            ...
            {/* Dynamic string conditional logic cleans up unwanted double bars rendering */}
            ...
            <p className="max-w-xl text-base text-gray-600 leading-relaxed sm:text-lg">
              ...
              undergraduate and postgraduate qualifications. Experience premium,
              flexible distance education from <strong>{uniFullName}</strong>{" "}
              built entirely around your routine.
            </p>
          ...
          {/* RIGHT COLUMN — Image Frame With Floating Admission Pill */}
          <div className="relative px-4 sm:px-0">
            <div className="relative z-10 overflow-hidden rounded-3xl shadow-2xl shadow-slate-200 ring-1 ring-slate-200">
              <img
                src={university?.image || defaultHeroImage}
                alt={`${uniFullName} Digital Campus`}
                className="h-[360px] w-full object-cover sm:h-[420px]"
              />
            </div>

            {/* Absolute Status Indicator Badge Floating Bottom Left Offset */}
            <div className="absolute -bottom-4 -left-2 sm:-left-4 z-20 rounded-2xl bg-white p-4 shadow-xl ring-1 ring-slate-100">
